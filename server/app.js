...
app.post("/login", async (req, res) => {    
  ...
  // console.log(users.findOne({ mail: req.body.mail }));
  ...
  users
    ...
    .then((user) => {
      ...
      if (!user)
        return res.send({
          message:
            "Error: The provided email is incorrect. Please check again.",
        });
      uInfo = user;
    ...
    .then((validPassword) => {
      ...
      if (!validPassword)
        return res.send({
          message:
            "Error: The provided password is incorrect. Please check again.",
        });
      req.session.userSession = uInfo;
      // console.log(req.session.userSession);
      // console.log("-----------------------------------------------");
      console.log(uInfo);
      res.send(uInfo);
